...
The GCC is a principled model of level of consciousness and global access. It asks when a heterogeneous, noisy, and structurally degradable network enters an integrated but non-rigid macroscopic regime. It is not a complete theory of phenomenal content or qualia; it is closer to global-access, integration/differentiation, metastability, perturbational-complexity, and clinical state-marker approaches \citep{dehaene1998,tononi2004,fries2015,kelso2012,deco2017,casali2013,sitt2014,engemann2018,demertzi2019,mashour2020,edlow2021}.

The evaluated core claim is that access-compatible dynamics occupy a bounded region jointly characterized by coherence, effective dimensionality, and temporal variability. This claim generates predictions P2--P6 and is tested through synthetic simulations plus public EEG analyses. A narrower degradation extension concerns selective re-integration in damaged systems:
...
This extension is conditional. It concerns degraded systems near the lower edge of the access region, not healthy systems that already lie stably within it. Its central empirical consequence is prediction~P1, but direct biological validation of P1 is not claimed here.

\paragraph{Three-stage evaluation.}
The paper separates three evaluation stages. Section~\ref{sec:numerics} tests model-internal consistency in synthetic Kuramoto networks, including a selective-preservation protocol that separates residual-backbone re-entry from generic retuning. Section~\ref{sec:pilot} applies the observables to two independent public propofol EEG datasets and adds narrower Sleep-EDF, ketamine, DoC, and dementia-proxy checks. Section~\ref{sec:spectral-controls} benchmarks GCC against spectral power, artifact controls, frozen cross-dataset transfer, lagged-coupling variants, phase-only and CSD/source-proxy implementations, and reliability analyses. These tests establish operational state sensitivity; they do not directly validate selective preservation in structurally degraded brains.
...
\noindent\textit{The following sections (\ref{sec:observablen}--\ref{sec:falsifizierbarkeit}) define the three operational observables, the calibrated access region, the operational access threshold, the basic mathematical properties of the dynamics, the evaluated prediction family, and the degradation/re-entry extension together with its falsifiability conditions. Additional formal material (proofs of well-posedness, bounds, Lipschitz continuity, stability, and the sufficient condition for re-entry) is provided in the Mathematical Appendix. Details of the EEG dataset, its ethical provenance, and the analysis pipeline are contained in the dataset subsection of the Pilot Demonstration (Section~\ref{sec:pilot}).}
...
For degradation and lucidity-like analyses, this separation is crucial.
...
The GCC contains a degradation extension: in systems that have left the access-compatible regime, selective preservation of efficacy on a residual subnetwork may permit a transient partial re-entry. This extension is developed here because it is the most distinctive mechanistic consequence of the model, but it should not be confused with the evaluated core of the present paper. The core evaluated claims are the regime predictions P2--P6 in synthetic simulations and the operational EEG pilot.

Lucidity-like re-entry remains only a possible future clinical application of this extension, not an empirical result established in the present work.
...
The model predicts a selective-preservation re-entry only if three conditions are simultaneously satisfied:
...
These conditions are intentionally strict. The model does not claim that every degraded or dying process produces a final moment of clarity.

\subsection{What the Model Explicitly Does Not Claim}

The model does \emph{not} claim
\begin{itemize}
\item that terminal or paradoxical lucidity is empirically demonstrated in the present work,
...
Six publishable, conservatively stated predictions follow from the model. In the present paper, P2--P6 form the evaluated prediction family. P1 is retained as the clinically important degradation extension: it is model-internally instantiated by P6 but not directly validated on structurally degraded biological data here.

\medskip
\noindent\textbf{P2.\;Critical range rather than arbitrary gradualism.} \emph{(Evaluated core prediction.)}\quad
...
\medskip
\noindent\textbf{P3.\;Degradation shifts the access region.} \emph{(Evaluated core prediction.)}\quad
In ensembles of degraded networks, $\Kcop$ typically shifts toward higher effective coupling or higher gain.

\medskip
\noindent\textbf{P4.\;Global access lies in an interval.} \emph{(Evaluated core prediction.)}\quad
...
\medskip
\noindent\textbf{P5.\;Anesthesia and awakening are trajectories in parameter space.} \emph{(Evaluated core prediction.)}\quad
State transitions can be described as motion in the space spanned by $W$, $G$, $g_i$, $\sigma_i$, and $\varphi_{ij}$.

\medskip
\noindent\textbf{P6.\;Selective preservation can produce residual-backbone re-entry in the model system.} \emph{(Evaluated core prediction.)}\quad
Under controlled degradation, selective preservation on a residual subnetwork should produce a higher access index on that subnetwork than uniform decline at matched global loss. This prediction is evaluated numerically and explicitly controlled against generic retuning.

\medskip
\noindent\textbf{P1.\;Functional re-integration through selective preservation in structurally degraded biological systems.} \emph{(Clinical extension.)}\quad
In severely degraded biological systems, states with reduced overall activity but elevated relative coherence within a residual subnetwork should correlate with improved global access. This prediction is counterintuitive because it contradicts models that correlate access monotonically with activity. It holds conditionally --- in degraded systems near the lower edge of the access region, not in healthy systems --- and requires direct validation in disorders-of-consciousness, dementia, or well-documented lucidity-like datasets.
...
\item Selective-preservation re-entry performs no better than uniform decline in controlled model systems and in future structurally degraded clinical datasets.
...
\textit{Nature Reviews Neurology}, \textbf{17}, 135--156.
...
\bibitem[Gramfort et~al.(2013)]{gramfort2013}
...
\textit{Frontiers in Neuroscience}, \textbf{7}, 267.
...
\bibitem[Pascual-Marqui(2002)]{pascualmarqui2002}
Pascual-Marqui,~R.\,D.\ (2002).
Standardized low-resolution brain electromagnetic tomography (sLORETA): technical details.
\textit{Methods and Findings in Experimental and Clinical Pharmacology}, \textbf{24 Suppl D}, 5--12.
