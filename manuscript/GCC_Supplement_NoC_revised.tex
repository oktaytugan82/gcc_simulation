...
\begin{sloppypar}
\noindent\textbf{S1 Code. GCC simulation and analysis code} (\texttt{gcc\_simulation\_v1.3.2.zip})\textbf{.} Complete Python codebase for the GCC dynamical model, per-subject EEG analysis pipeline, Chennu gamma baseline-holdout analysis (\texttt{run\_chennu\_baseline\_holdout\_from\_observables.py}), Hermann DoC proxy analysis (\texttt{run\_hermann2021\_gcc\_proxy\_analysis.py}), V3/V4 ensemble simulation (\texttt{test\_v2\_v3\_ensemble.py}), V6 retuning-control sweep (\texttt{test\_v5\_retuning\_control.py}), mixed-effects statistical analysis over the sweep (\texttt{test\_v5\_mixed\_effects.py}), and figure-generation scripts. Includes \texttt{requirements.txt} and \texttt{README.md} with reproduction instructions. Corresponds to frozen GitHub release/tag \texttt{v1.3.2} at \url{https://github.com/oktaytugan82/gcc_simulation/tree/v1.3.2}; the project-level Zenodo concept DOI is \href{https://doi.org/10.5281/zenodo.19744969}{10.5281/zenodo.19744969}.
\end{sloppypar}
